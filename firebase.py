        ...
        for doc in docs:
            if isinstance(doc, firestore.firestore.DocumentSnapshot):
                ...
                if isinstance(post['date'], str):
                    post['date'] = datetime.strptime(post['date'], '%Y-%m-%dT%H:%M:%S.%fZ')
                results.append(post)
            else:
...
def get_comments_from_post(post_id):
    got = db.collection('Comments').where('postID', '==', post_id).order_by("date", direction=firestore.Query.DESCENDING).stream()
    comms_list = [doc.to_dict() for doc in got]
    return comms_list
...
def create_post(userID, author, title, content, font='monospace'):
    try:
        if userID and author and title and content:
