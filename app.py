...
from firebase import ( get_recent_posts, get_popular_posts, get_user_posts, get_all_posts,
                       search_posts, get_user_id_from_auth, get_comments_from_post,
...
# DISCUSS -- shows all posts for now
@app.route('/discuss', methods=['GET'])
def api_discuss():
    all_posts = get_all_posts()
    with app.app_context():
        return render_template('discuss.html', all_posts=all_posts, total_posts=len(all_posts)), 200

# GET COMMENTS FOR A POST BY POST ID -- TODO -- 
@app.route('/id/<string:post_id>/Comments', methods=['GET', 'POST'])
def view_comment(post_id):
    try:
        post = get_post_by_post_id(post_id)
        comments = get_comments_from_post(post_id)
        with app.app_context():
            return render_template('discuss.html', post=post), 200
    except Exception as e:
...
def api_create_post():
    if request.method == 'POST' :
        try:
            ...
            author = request.form.get('author', 'anon') # default userID, replace with auth
            title = request.form.get('title')
            content = request.form.get('content').replace('\n', '<br>').replace('\t', '&emsp')
            font = request.form.get('font', 'monospace') # default font
            post_id = create_post(userID, author, title, content, font)
